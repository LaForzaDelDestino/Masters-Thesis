...
while a < size(Variable,2)+1
    ...
    while b < size(model,2)+1
        ...
        if Variable(a) == "VPD"
            levels = [-250:250:2500];
            axis_spread = [-250 2500];
            ticks = [-250, 0:500:2500];
        ...
        elseif Variable(a) == "pr"
            levels = [-10^.75, -10^.5, -10^.25, -10^0, -10^-.25, -10^-.5, 0, 10^-.5, 10^-.25, 10^0, 10^.25, 10^.5, 10^.75];
        end
        
        contourfm(lat, lon, diff, levels, "LineWidth", 0.1, "Color", [1 1 1]);
        ...
        if Variable(a) == "pr"
            ...
            cb.Ticks = [-10^.75, -10^.5, -10^.25, -10^0, 0, 10^0, 10^.25, 10^.5, 10^.75];
        else
            caxis(axis_spread)
            contourcmap("jet")
            cb = contourcbar("eastoutside");
    ...
    sg = sprintf("Maps: Differences\n%s",Variable_name(a));
    sgtitle(sg,'FontSize',24)
    
    % Set figure size in inches
    fig.Units = 'inches';
    fig.Position = [1, 1, 16, 12];  % [left, bottom, width, height] => 16x16 inches
    saveas(fig, '/home/disk/eos8/dglopez/pictures/Differences_Variable/Differences_Hemisphere_Map_'+Variable(a)+'_All.png');
